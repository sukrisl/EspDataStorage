...
extern "C" void app_main(void) {
    ...
    exFS = storage.mount("exFS", "/exFS", true);
    ...
    // Read string until from file
    char buffer[500] = {0};
    char bufferIn[500] = {0};
    storage.read(exFS, "/data.txt", buffer, sizeof(buffer), ' ', 4);
    storage.read(inFS, "/data.txt", bufferIn, sizeof(bufferIn), ' ', 4);
    ESP_LOGI(TAG, "File content external:\n%s", buffer);
    ESP_LOGI(TAG, "File content internal:\n%s", bufferIn);
